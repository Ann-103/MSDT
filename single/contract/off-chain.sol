...
contract Launchpad{
    ...
    *
    */
    function requestService(uint[] memory setSeq, uint payNum, bytes32 payHash) payable public {
    ...
    *
    */
    function cloudTerminate(bytes32 paySeed, uint setNum) public {
        require(msg.sender == _cloud, "Should be the correct client address");
        require(_serviceStatus == 1, "Error service sequence");
        require(now <= _requestStartTime + _duringTime, 'The terminate time is over');
        
        for (uint i = 1; i <= setNum; i++) {
            paySeed = sha256(abi.encodePacked(paySeed));
        }
        require(_payHash == paySeed, "Error payment seed");
        payable(_cloud).transfer(setNum * _clientServiceFee);
        payable(_client).transfer((_payNum - setNum) * _clientServiceFee);
        _serviceStatus = 2;
    ...
    *
    */
    function cloudArbitrate(string memory s2, bytes32 hashOfS1, bytes memory sig, uint setNum, bytes32 newResult) public {
        require(msg.sender == _cloud, "Should be the correct client address");
        require(_serviceStatus == 1, "Error service sequence");
        require(now <= _requestStartTime + _duringTime, 'The terminate time is over');
        uint n = _setSeq[(setNum - 1) % _n] % _n;
        require((hashOfS1 ^ sha256(bytes(s2))) == _verifyResult[n], "Arbitrate fail");
        require(_client == ecrecovery(hashOfS1,sig), "Wrong signature");
    ...
    */
    function clientTerminate() public {
        require(msg.sender == _client, "Should be the correct address");
        require(_serviceStatus == 1, "Error service sequence");
        require(now > _requestStartTime + _duringTime, 'The terminate time is not over');
            
        _serviceStatus = 2;
        payable(_client).transfer(_payNum * _clientServiceFee);
